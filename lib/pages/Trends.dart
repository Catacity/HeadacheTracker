...
class Trends extends StatelessWidget {
  ...
  const Trends({Key? key, required this.userID}) : super(key: key);
              ...
              child: TextButton(
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => HeadacheTrendsPage.async(key: ValueKey(now), userID: userID),
                    ),
                  );
                },
                child: Column(
...
class _HeadacheTrendsPageState extends State<HeadacheTrendsPage> {
  ...
  Future<File> _loadHtmlFromAssets() async {
    ...
    File tempFile = File('${tempDir.path}/temp.html');
    await tempFile.writeAsString(
      fileText.replaceAll('{{data}}', await dataToJson()), // Replace {{data}} with JSON data
      flush: true,
    );
    return tempFile;
  }

  Future<String> dataToJson() async{
    ...
    setState(() {
      reformattedString = jsonEncode(data);
    });
          ...
          Row(
            ...
            children: [
              customButton(context, 'Intensity', 'lib/images/intensity.png',  () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => HeadacheIntensityPage(jsonData:reformattedString)));
              }),

              customButton(context, 'Medicine', 'lib/images/pill.png',  () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => HeadacheMedicinePage(jsonData:reformattedString)));
              }),
              customButton(context, 'Symptom', 'lib/images/symptom.png',  () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => SymptomsIntensityPage(jsonData:reformattedString)));
              }),
            ],
          ),
