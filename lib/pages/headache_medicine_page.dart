...
class HeadacheMedicinePage extends StatefulWidget {
  final String? jsonData;

  const HeadacheMedicinePage({Key? key, required this.jsonData}) : super(key: key);

  factory HeadacheMedicinePage.async({required Key key, required String? jsonData}) {

    return HeadacheMedicinePage(
        key: key,
        jsonData: jsonData
    );
  }
  ...
  @override
  _HeadacheMedicinePageState createState() => _HeadacheMedicinePageState();
}

class _HeadacheMedicinePageState extends State<HeadacheMedicinePage> {
  ...
  Future<File> _loadHtmlFromAssets() async {
    ...
    String fileText = await rootBundle.loadString('assets/vega_lite3.html');
